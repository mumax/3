...
var (
	...
	NoDemagSpins  = NewScalarInput("NoDemagSpins", "", "Disable magnetostatic interaction per-spin (set to 1 to disable)")
...
func setMaskedDemagField(dst *data.Slice, msat cuda.MSlice) {
	...
	// mask-out
	mask := MSliceOf(NoDemagSpins)
	defer mask.Recycle()
	cuda.ZeroMask(buf, mask)
