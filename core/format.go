...
func Fprintf(out io.Writer, format string, array ...interface{}){
	...
	switch a:=arr.(type){
		case [][][]float32: FprintfFloats(out, format, a)
		case [][][][]float32: FprintfTensors(out, format, a)
		case [3][][][]float32: FprintfTensors(out, format, a[:])
		default: fmt.Fprintf(out, format, a)
	}
	}
}
