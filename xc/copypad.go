...
func copyPad(dst, src safe.Float32s, dstsize, srcsize, offset [3]int, stream cu.Stream) {
	...
	core.Assert(srcsize[0] <= dstsize[0] && srcsize[1] <= dstsize[1] && srcsize[2] <= dstsize[2])
...
// Copies src into dst (which is smaller), at offset position.
func copyUnpad(dst, src safe.Float32s, dstsize, srcsize, offset [3]int, stream cu.Stream) {
	core.Assert(dst.Len() == prod(dstsize))
	core.Assert(src.Len() == prod(srcsize))
	core.Assert(srcsize[0] >= dstsize[0] && srcsize[1] >= dstsize[1] && srcsize[2] >= dstsize[2])
	// TODO: either remove offset or check offset
	...
	block := 16
	gridJ := DivUp(dstsize[1], block)
	gridK := DivUp(dstsize[2], block)
	shmem := 0
	args := []unsafe.Pointer{
		unsafe.Pointer(&dstptr),
		unsafe.Pointer(&dstsize[0]),
