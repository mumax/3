package cuda

/*
 THIS FILE IS AUTO-GENERATED BY CUDA2GO.
 EDITING IS FUTILE.
*/

import (
	"github.com/barnex/cuda5/cu"
	"unsafe"
)

var kernmulRSymm3D_code cu.Function

type kernmulRSymm3D_args struct {
	arg_fftMx  unsafe.Pointer
	arg_fftMy  unsafe.Pointer
	arg_fftMz  unsafe.Pointer
	arg_fftKxx unsafe.Pointer
	arg_fftKyy unsafe.Pointer
	arg_fftKzz unsafe.Pointer
	arg_fftKyz unsafe.Pointer
	arg_fftKxz unsafe.Pointer
	arg_fftKxy unsafe.Pointer
	arg_Nx     int
	arg_Ny     int
	arg_Nz     int
	argptr     [12]unsafe.Pointer
}

// Wrapper for kernmulRSymm3D CUDA kernel, asynchronous.
func k_kernmulRSymm3D_async(fftMx unsafe.Pointer, fftMy unsafe.Pointer, fftMz unsafe.Pointer, fftKxx unsafe.Pointer, fftKyy unsafe.Pointer, fftKzz unsafe.Pointer, fftKyz unsafe.Pointer, fftKxz unsafe.Pointer, fftKxy unsafe.Pointer, Nx int, Ny int, Nz int, cfg *config, str int) {
	if kernmulRSymm3D_code == 0 {
		kernmulRSymm3D_code = fatbinLoad(kernmulRSymm3D_map, "kernmulRSymm3D")
	}

	var _a_ kernmulRSymm3D_args

	_a_.arg_fftMx = fftMx
	_a_.argptr[0] = unsafe.Pointer(&_a_.arg_fftMx)
	_a_.arg_fftMy = fftMy
	_a_.argptr[1] = unsafe.Pointer(&_a_.arg_fftMy)
	_a_.arg_fftMz = fftMz
	_a_.argptr[2] = unsafe.Pointer(&_a_.arg_fftMz)
	_a_.arg_fftKxx = fftKxx
	_a_.argptr[3] = unsafe.Pointer(&_a_.arg_fftKxx)
	_a_.arg_fftKyy = fftKyy
	_a_.argptr[4] = unsafe.Pointer(&_a_.arg_fftKyy)
	_a_.arg_fftKzz = fftKzz
	_a_.argptr[5] = unsafe.Pointer(&_a_.arg_fftKzz)
	_a_.arg_fftKyz = fftKyz
	_a_.argptr[6] = unsafe.Pointer(&_a_.arg_fftKyz)
	_a_.arg_fftKxz = fftKxz
	_a_.argptr[7] = unsafe.Pointer(&_a_.arg_fftKxz)
	_a_.arg_fftKxy = fftKxy
	_a_.argptr[8] = unsafe.Pointer(&_a_.arg_fftKxy)
	_a_.arg_Nx = Nx
	_a_.argptr[9] = unsafe.Pointer(&_a_.arg_Nx)
	_a_.arg_Ny = Ny
	_a_.argptr[10] = unsafe.Pointer(&_a_.arg_Ny)
	_a_.arg_Nz = Nz
	_a_.argptr[11] = unsafe.Pointer(&_a_.arg_Nz)

	args := _a_.argptr[:]
	cu.LaunchKernel(kernmulRSymm3D_code, cfg.Grid.X, cfg.Grid.Y, cfg.Grid.Z, cfg.Block.X, cfg.Block.Y, cfg.Block.Z, 0, stream[str], args)
}

// Wrapper for kernmulRSymm3D CUDA kernel, synchronized.
func k_kernmulRSymm3D(fftMx unsafe.Pointer, fftMy unsafe.Pointer, fftMz unsafe.Pointer, fftKxx unsafe.Pointer, fftKyy unsafe.Pointer, fftKzz unsafe.Pointer, fftKyz unsafe.Pointer, fftKxz unsafe.Pointer, fftKxy unsafe.Pointer, Nx int, Ny int, Nz int, cfg *config) {
	const stream = 0
	k_kernmulRSymm3D_async(fftMx, fftMy, fftMz, fftKxx, fftKyy, fftKzz, fftKyz, fftKxz, fftKxy, Nx, Ny, Nz, cfg, stream)
	Sync(stream)
}

var kernmulRSymm3D_map = map[int]string{0: "",
	20: kernmulRSymm3D_ptx_20,
	30: kernmulRSymm3D_ptx_30,
	35: kernmulRSymm3D_ptx_35}

const (
	kernmulRSymm3D_ptx_20 = `
.version 3.1
.target sm_20
.address_size 64


.visible .entry kernmulRSymm3D(
	.param .u64 kernmulRSymm3D_param_0,
	.param .u64 kernmulRSymm3D_param_1,
	.param .u64 kernmulRSymm3D_param_2,
	.param .u64 kernmulRSymm3D_param_3,
	.param .u64 kernmulRSymm3D_param_4,
	.param .u64 kernmulRSymm3D_param_5,
	.param .u64 kernmulRSymm3D_param_6,
	.param .u64 kernmulRSymm3D_param_7,
	.param .u64 kernmulRSymm3D_param_8,
	.param .u32 kernmulRSymm3D_param_9,
	.param .u32 kernmulRSymm3D_param_10,
	.param .u32 kernmulRSymm3D_param_11
)
{
	.reg .pred 	%p<7>;
	.reg .s32 	%r<86>;
	.reg .f32 	%f<39>;
	.reg .s64 	%rd<43>;


	ld.param.u64 	%rd2, [kernmulRSymm3D_param_0];
	ld.param.u64 	%rd3, [kernmulRSymm3D_param_1];
	ld.param.u64 	%rd4, [kernmulRSymm3D_param_2];
	ld.param.u64 	%rd5, [kernmulRSymm3D_param_3];
	ld.param.u64 	%rd6, [kernmulRSymm3D_param_4];
	ld.param.u64 	%rd7, [kernmulRSymm3D_param_5];
	ld.param.u64 	%rd8, [kernmulRSymm3D_param_6];
	ld.param.u64 	%rd9, [kernmulRSymm3D_param_7];
	ld.param.u64 	%rd10, [kernmulRSymm3D_param_8];
	ld.param.u32 	%r8, [kernmulRSymm3D_param_9];
	ld.param.u32 	%r9, [kernmulRSymm3D_param_10];
	ld.param.u32 	%r10, [kernmulRSymm3D_param_11];
	cvta.to.global.u64 	%rd1, %rd4;
	.loc 2 35 1
	mov.u32 	%r11, %ntid.x;
	mov.u32 	%r12, %ctaid.x;
	mov.u32 	%r13, %tid.x;
	mad.lo.s32 	%r1, %r11, %r12, %r13;
	.loc 2 36 1
	mov.u32 	%r14, %ntid.y;
	mov.u32 	%r15, %ctaid.y;
	mov.u32 	%r16, %tid.y;
	mad.lo.s32 	%r2, %r14, %r15, %r16;
	.loc 2 37 1
	mov.u32 	%r17, %ntid.z;
	mov.u32 	%r18, %ctaid.z;
	mov.u32 	%r19, %tid.z;
	mad.lo.s32 	%r3, %r17, %r18, %r19;
	.loc 2 39 1
	setp.ge.s32 	%p1, %r2, %r9;
	setp.ge.s32 	%p2, %r1, %r8;
	or.pred  	%p3, %p1, %p2;
	setp.ge.s32 	%p4, %r3, %r10;
	or.pred  	%p5, %p3, %p4;
	@%p5 bra 	BB0_5;

	.loc 2 45 1
	mul.lo.s32 	%r4, %r3, %r9;
	mad.lo.s32 	%r20, %r3, %r9, %r2;
	mad.lo.s32 	%r85, %r20, %r8, %r1;
	.loc 2 46 1
	shl.b32 	%r22, %r85, 1;
	cvta.to.global.u64 	%rd11, %rd2;
	.loc 2 47 1
	mul.wide.s32 	%rd12, %r22, 4;
	add.s64 	%rd13, %rd11, %rd12;
	ld.global.f32 	%f1, [%rd13];
	.loc 2 48 1
	ld.global.f32 	%f2, [%rd13+4];
	cvta.to.global.u64 	%rd14, %rd3;
	.loc 2 49 1
	add.s64 	%rd15, %rd14, %rd12;
	ld.global.f32 	%f3, [%rd15];
	.loc 2 50 1
	ld.global.f32 	%f4, [%rd15+4];
	.loc 2 51 1
	add.s64 	%rd16, %rd1, %rd12;
	ld.global.f32 	%f5, [%rd16];
	.loc 2 52 1
	ld.global.f32 	%f6, [%rd16+4];
	.loc 2 54 1
	shr.u32 	%r29, %r9, 31;
	add.s32 	%r30, %r9, %r29;
	shr.s32 	%r31, %r30, 1;
	add.s32 	%r32, %r31, 1;
	setp.lt.s32 	%p6, %r2, %r32;
	@%p6 bra 	BB0_3;

	.loc 2 62 1
	sub.s32 	%r37, %r9, %r2;
	add.s32 	%r38, %r37, %r4;
	mad.lo.s32 	%r85, %r38, %r8, %r1;
	cvta.to.global.u64 	%rd17, %rd8;
	.loc 2 66 1
	mul.wide.s32 	%rd18, %r85, 4;
	add.s64 	%rd19, %rd17, %rd18;
	ld.global.f32 	%f13, [%rd19];
	neg.f32 	%f38, %f13;
	cvta.to.global.u64 	%rd20, %rd10;
	.loc 2 68 1
	add.s64 	%rd21, %rd20, %rd18;
	ld.global.f32 	%f14, [%rd21];
	neg.f32 	%f37, %f14;
	bra.uni 	BB0_4;

BB0_3:
	cvta.to.global.u64 	%rd22, %rd8;
	.loc 2 58 1
	mul.wide.s32 	%rd23, %r85, 4;
	add.s64 	%rd24, %rd22, %rd23;
	ld.global.f32 	%f38, [%rd24];
	cvta.to.global.u64 	%rd25, %rd10;
	.loc 2 60 1
	add.s64 	%rd26, %rd25, %rd23;
	ld.global.f32 	%f37, [%rd26];

BB0_4:
	cvta.to.global.u64 	%rd27, %rd9;
	.loc 2 59 1
	mul.wide.s32 	%rd28, %r85, 4;
	add.s64 	%rd29, %rd27, %rd28;
	cvta.to.global.u64 	%rd30, %rd7;
	.loc 2 57 1
	add.s64 	%rd31, %rd30, %rd28;
	cvta.to.global.u64 	%rd32, %rd6;
	.loc 2 56 1
	add.s64 	%rd33, %rd32, %rd28;
	cvta.to.global.u64 	%rd34, %rd5;
	.loc 2 55 1
	add.s64 	%rd35, %rd34, %rd28;
	.loc 2 71 1
	ld.global.f32 	%f15, [%rd31];
	ld.global.f32 	%f16, [%rd33];
	ld.global.f32 	%f17, [%rd35];
	mul.f32 	%f18, %f3, %f37;
	fma.rn.f32 	%f19, %f1, %f17, %f18;
	ld.global.f32 	%f20, [%rd29];
	fma.rn.f32 	%f21, %f5, %f20, %f19;
	.loc 2 47 1
	mul.wide.s32 	%rd37, %r22, 4;
	add.s64 	%rd38, %rd11, %rd37;
	.loc 2 71 1
	st.global.f32 	[%rd38], %f21;
	.loc 2 72 1
	mul.f32 	%f22, %f4, %f37;
	fma.rn.f32 	%f23, %f2, %f17, %f22;
	fma.rn.f32 	%f24, %f6, %f20, %f23;
	st.global.f32 	[%rd38+4], %f24;
	.loc 2 73 1
	mul.f32 	%f25, %f3, %f16;
	fma.rn.f32 	%f26, %f1, %f37, %f25;
	fma.rn.f32 	%f27, %f5, %f38, %f26;
	.loc 2 49 1
	add.s64 	%rd40, %rd14, %rd37;
	.loc 2 73 1
	st.global.f32 	[%rd40], %f27;
	.loc 2 74 1
	mul.f32 	%f28, %f4, %f16;
	fma.rn.f32 	%f29, %f2, %f37, %f28;
	fma.rn.f32 	%f30, %f6, %f38, %f29;
	st.global.f32 	[%rd40+4], %f30;
	.loc 2 75 1
	mul.f32 	%f31, %f3, %f38;
	fma.rn.f32 	%f32, %f1, %f20, %f31;
	fma.rn.f32 	%f33, %f5, %f15, %f32;
	.loc 2 51 1
	add.s64 	%rd42, %rd1, %rd37;
	.loc 2 75 1
	st.global.f32 	[%rd42], %f33;
	.loc 2 76 1
	mul.f32 	%f34, %f4, %f38;
	fma.rn.f32 	%f35, %f2, %f20, %f34;
	fma.rn.f32 	%f36, %f6, %f15, %f35;
	st.global.f32 	[%rd42+4], %f36;

BB0_5:
	.loc 2 77 2
	ret;
}


`
	kernmulRSymm3D_ptx_30 = `
.version 3.1
.target sm_30
.address_size 64


.visible .entry kernmulRSymm3D(
	.param .u64 kernmulRSymm3D_param_0,
	.param .u64 kernmulRSymm3D_param_1,
	.param .u64 kernmulRSymm3D_param_2,
	.param .u64 kernmulRSymm3D_param_3,
	.param .u64 kernmulRSymm3D_param_4,
	.param .u64 kernmulRSymm3D_param_5,
	.param .u64 kernmulRSymm3D_param_6,
	.param .u64 kernmulRSymm3D_param_7,
	.param .u64 kernmulRSymm3D_param_8,
	.param .u32 kernmulRSymm3D_param_9,
	.param .u32 kernmulRSymm3D_param_10,
	.param .u32 kernmulRSymm3D_param_11
)
{
	.reg .pred 	%p<7>;
	.reg .s32 	%r<49>;
	.reg .f32 	%f<39>;
	.reg .s64 	%rd<43>;


	ld.param.u64 	%rd4, [kernmulRSymm3D_param_0];
	ld.param.u64 	%rd5, [kernmulRSymm3D_param_1];
	ld.param.u64 	%rd6, [kernmulRSymm3D_param_2];
	ld.param.u64 	%rd7, [kernmulRSymm3D_param_3];
	ld.param.u64 	%rd8, [kernmulRSymm3D_param_4];
	ld.param.u64 	%rd9, [kernmulRSymm3D_param_5];
	ld.param.u64 	%rd10, [kernmulRSymm3D_param_6];
	ld.param.u64 	%rd11, [kernmulRSymm3D_param_7];
	ld.param.u64 	%rd12, [kernmulRSymm3D_param_8];
	ld.param.u32 	%r9, [kernmulRSymm3D_param_9];
	ld.param.u32 	%r10, [kernmulRSymm3D_param_10];
	ld.param.u32 	%r11, [kernmulRSymm3D_param_11];
	cvta.to.global.u64 	%rd1, %rd6;
	cvta.to.global.u64 	%rd2, %rd5;
	cvta.to.global.u64 	%rd3, %rd4;
	.loc 2 35 1
	mov.u32 	%r12, %ntid.x;
	mov.u32 	%r13, %ctaid.x;
	mov.u32 	%r14, %tid.x;
	mad.lo.s32 	%r1, %r12, %r13, %r14;
	.loc 2 36 1
	mov.u32 	%r15, %ntid.y;
	mov.u32 	%r16, %ctaid.y;
	mov.u32 	%r17, %tid.y;
	mad.lo.s32 	%r2, %r15, %r16, %r17;
	.loc 2 37 1
	mov.u32 	%r18, %ntid.z;
	mov.u32 	%r19, %ctaid.z;
	mov.u32 	%r20, %tid.z;
	mad.lo.s32 	%r3, %r18, %r19, %r20;
	.loc 2 39 1
	setp.ge.s32 	%p1, %r2, %r10;
	setp.ge.s32 	%p2, %r1, %r9;
	or.pred  	%p3, %p1, %p2;
	setp.ge.s32 	%p4, %r3, %r11;
	or.pred  	%p5, %p3, %p4;
	@%p5 bra 	BB0_5;

	.loc 2 45 1
	mul.lo.s32 	%r4, %r3, %r10;
	mad.lo.s32 	%r21, %r3, %r10, %r2;
	mad.lo.s32 	%r48, %r21, %r9, %r1;
	.loc 2 46 1
	shl.b32 	%r6, %r48, 1;
	.loc 2 47 1
	mul.wide.s32 	%rd13, %r6, 4;
	add.s64 	%rd14, %rd3, %rd13;
	ld.global.f32 	%f1, [%rd14];
	.loc 2 48 1
	ld.global.f32 	%f2, [%rd14+4];
	.loc 2 49 1
	add.s64 	%rd15, %rd2, %rd13;
	ld.global.f32 	%f3, [%rd15];
	.loc 2 50 1
	ld.global.f32 	%f4, [%rd15+4];
	.loc 2 51 1
	add.s64 	%rd16, %rd1, %rd13;
	ld.global.f32 	%f5, [%rd16];
	.loc 2 52 1
	ld.global.f32 	%f6, [%rd16+4];
	.loc 2 54 1
	shr.u32 	%r28, %r10, 31;
	add.s32 	%r29, %r10, %r28;
	shr.s32 	%r30, %r29, 1;
	add.s32 	%r31, %r30, 1;
	setp.lt.s32 	%p6, %r2, %r31;
	@%p6 bra 	BB0_3;

	.loc 2 62 1
	sub.s32 	%r32, %r10, %r2;
	add.s32 	%r33, %r32, %r4;
	mad.lo.s32 	%r48, %r33, %r9, %r1;
	cvta.to.global.u64 	%rd17, %rd10;
	.loc 2 66 1
	mul.wide.s32 	%rd18, %r48, 4;
	add.s64 	%rd19, %rd17, %rd18;
	ld.global.f32 	%f13, [%rd19];
	neg.f32 	%f38, %f13;
	cvta.to.global.u64 	%rd20, %rd12;
	.loc 2 68 1
	add.s64 	%rd21, %rd20, %rd18;
	ld.global.f32 	%f14, [%rd21];
	neg.f32 	%f37, %f14;
	bra.uni 	BB0_4;

BB0_3:
	cvta.to.global.u64 	%rd22, %rd10;
	.loc 2 58 1
	mul.wide.s32 	%rd23, %r48, 4;
	add.s64 	%rd24, %rd22, %rd23;
	ld.global.f32 	%f38, [%rd24];
	cvta.to.global.u64 	%rd25, %rd12;
	.loc 2 60 1
	add.s64 	%rd26, %rd25, %rd23;
	ld.global.f32 	%f37, [%rd26];

BB0_4:
	cvta.to.global.u64 	%rd27, %rd11;
	.loc 2 59 1
	mul.wide.s32 	%rd28, %r48, 4;
	add.s64 	%rd29, %rd27, %rd28;
	cvta.to.global.u64 	%rd30, %rd9;
	.loc 2 57 1
	add.s64 	%rd31, %rd30, %rd28;
	cvta.to.global.u64 	%rd32, %rd8;
	.loc 2 56 1
	add.s64 	%rd33, %rd32, %rd28;
	cvta.to.global.u64 	%rd34, %rd7;
	.loc 2 55 1
	add.s64 	%rd35, %rd34, %rd28;
	.loc 2 71 1
	ld.global.f32 	%f15, [%rd31];
	ld.global.f32 	%f16, [%rd33];
	ld.global.f32 	%f17, [%rd35];
	mul.f32 	%f18, %f3, %f37;
	fma.rn.f32 	%f19, %f1, %f17, %f18;
	ld.global.f32 	%f20, [%rd29];
	fma.rn.f32 	%f21, %f5, %f20, %f19;
	.loc 2 47 1
	mul.wide.s32 	%rd37, %r6, 4;
	add.s64 	%rd38, %rd3, %rd37;
	.loc 2 71 1
	st.global.f32 	[%rd38], %f21;
	.loc 2 72 1
	mul.f32 	%f22, %f4, %f37;
	fma.rn.f32 	%f23, %f2, %f17, %f22;
	fma.rn.f32 	%f24, %f6, %f20, %f23;
	st.global.f32 	[%rd38+4], %f24;
	.loc 2 73 1
	mul.f32 	%f25, %f3, %f16;
	fma.rn.f32 	%f26, %f1, %f37, %f25;
	fma.rn.f32 	%f27, %f5, %f38, %f26;
	.loc 2 49 1
	add.s64 	%rd40, %rd2, %rd37;
	.loc 2 73 1
	st.global.f32 	[%rd40], %f27;
	.loc 2 74 1
	mul.f32 	%f28, %f4, %f16;
	fma.rn.f32 	%f29, %f2, %f37, %f28;
	fma.rn.f32 	%f30, %f6, %f38, %f29;
	st.global.f32 	[%rd40+4], %f30;
	.loc 2 75 1
	mul.f32 	%f31, %f3, %f38;
	fma.rn.f32 	%f32, %f1, %f20, %f31;
	fma.rn.f32 	%f33, %f5, %f15, %f32;
	.loc 2 51 1
	add.s64 	%rd42, %rd1, %rd37;
	.loc 2 75 1
	st.global.f32 	[%rd42], %f33;
	.loc 2 76 1
	mul.f32 	%f34, %f4, %f38;
	fma.rn.f32 	%f35, %f2, %f20, %f34;
	fma.rn.f32 	%f36, %f6, %f15, %f35;
	st.global.f32 	[%rd42+4], %f36;

BB0_5:
	.loc 2 77 2
	ret;
}


`
	kernmulRSymm3D_ptx_35 = `
.version 3.1
.target sm_35
.address_size 64


.weak .func  (.param .b32 func_retval0) cudaMalloc(
	.param .b64 cudaMalloc_param_0,
	.param .b64 cudaMalloc_param_1
)
{
	.reg .s32 	%r<2>;


	mov.u32 	%r1, 30;
	st.param.b32	[func_retval0+0], %r1;
	.loc 2 66 3
	ret;
}

.weak .func  (.param .b32 func_retval0) cudaFuncGetAttributes(
	.param .b64 cudaFuncGetAttributes_param_0,
	.param .b64 cudaFuncGetAttributes_param_1
)
{
	.reg .s32 	%r<2>;


	mov.u32 	%r1, 30;
	st.param.b32	[func_retval0+0], %r1;
	.loc 2 71 3
	ret;
}

.visible .entry kernmulRSymm3D(
	.param .u64 kernmulRSymm3D_param_0,
	.param .u64 kernmulRSymm3D_param_1,
	.param .u64 kernmulRSymm3D_param_2,
	.param .u64 kernmulRSymm3D_param_3,
	.param .u64 kernmulRSymm3D_param_4,
	.param .u64 kernmulRSymm3D_param_5,
	.param .u64 kernmulRSymm3D_param_6,
	.param .u64 kernmulRSymm3D_param_7,
	.param .u64 kernmulRSymm3D_param_8,
	.param .u32 kernmulRSymm3D_param_9,
	.param .u32 kernmulRSymm3D_param_10,
	.param .u32 kernmulRSymm3D_param_11
)
{
	.reg .pred 	%p<7>;
	.reg .s32 	%r<41>;
	.reg .f32 	%f<39>;
	.reg .s64 	%rd<43>;


	ld.param.u64 	%rd4, [kernmulRSymm3D_param_0];
	ld.param.u64 	%rd5, [kernmulRSymm3D_param_1];
	ld.param.u64 	%rd6, [kernmulRSymm3D_param_2];
	ld.param.u64 	%rd7, [kernmulRSymm3D_param_3];
	ld.param.u64 	%rd8, [kernmulRSymm3D_param_4];
	ld.param.u64 	%rd9, [kernmulRSymm3D_param_5];
	ld.param.u64 	%rd10, [kernmulRSymm3D_param_6];
	ld.param.u64 	%rd11, [kernmulRSymm3D_param_7];
	ld.param.u64 	%rd12, [kernmulRSymm3D_param_8];
	ld.param.u32 	%r9, [kernmulRSymm3D_param_9];
	ld.param.u32 	%r10, [kernmulRSymm3D_param_10];
	ld.param.u32 	%r11, [kernmulRSymm3D_param_11];
	cvta.to.global.u64 	%rd1, %rd6;
	cvta.to.global.u64 	%rd2, %rd5;
	cvta.to.global.u64 	%rd3, %rd4;
	.loc 3 35 1
	mov.u32 	%r12, %ntid.x;
	mov.u32 	%r13, %ctaid.x;
	mov.u32 	%r14, %tid.x;
	mad.lo.s32 	%r1, %r12, %r13, %r14;
	.loc 3 36 1
	mov.u32 	%r15, %ntid.y;
	mov.u32 	%r16, %ctaid.y;
	mov.u32 	%r17, %tid.y;
	mad.lo.s32 	%r2, %r15, %r16, %r17;
	.loc 3 37 1
	mov.u32 	%r18, %ntid.z;
	mov.u32 	%r19, %ctaid.z;
	mov.u32 	%r20, %tid.z;
	mad.lo.s32 	%r3, %r18, %r19, %r20;
	.loc 3 39 1
	setp.ge.s32 	%p1, %r2, %r10;
	setp.ge.s32 	%p2, %r1, %r9;
	or.pred  	%p3, %p1, %p2;
	setp.ge.s32 	%p4, %r3, %r11;
	or.pred  	%p5, %p3, %p4;
	@%p5 bra 	BB2_5;

	.loc 3 45 1
	mul.lo.s32 	%r4, %r3, %r10;
	mad.lo.s32 	%r21, %r3, %r10, %r2;
	mad.lo.s32 	%r40, %r21, %r9, %r1;
	.loc 3 46 1
	shl.b32 	%r6, %r40, 1;
	.loc 3 47 1
	mul.wide.s32 	%rd13, %r6, 4;
	add.s64 	%rd14, %rd3, %rd13;
	ld.global.f32 	%f1, [%rd14];
	.loc 3 48 1
	ld.global.f32 	%f2, [%rd14+4];
	.loc 3 49 1
	add.s64 	%rd15, %rd2, %rd13;
	ld.global.f32 	%f3, [%rd15];
	.loc 3 50 1
	ld.global.f32 	%f4, [%rd15+4];
	.loc 3 51 1
	add.s64 	%rd16, %rd1, %rd13;
	ld.global.f32 	%f5, [%rd16];
	.loc 3 52 1
	ld.global.f32 	%f6, [%rd16+4];
	.loc 3 54 1
	shr.u32 	%r28, %r10, 31;
	add.s32 	%r29, %r10, %r28;
	shr.s32 	%r30, %r29, 1;
	add.s32 	%r31, %r30, 1;
	setp.lt.s32 	%p6, %r2, %r31;
	@%p6 bra 	BB2_3;

	.loc 3 62 1
	sub.s32 	%r32, %r10, %r2;
	add.s32 	%r33, %r32, %r4;
	mad.lo.s32 	%r40, %r33, %r9, %r1;
	cvta.to.global.u64 	%rd17, %rd10;
	.loc 3 66 1
	mul.wide.s32 	%rd18, %r40, 4;
	add.s64 	%rd19, %rd17, %rd18;
	ld.global.nc.f32 	%f13, [%rd19];
	neg.f32 	%f38, %f13;
	cvta.to.global.u64 	%rd20, %rd12;
	.loc 3 68 1
	add.s64 	%rd21, %rd20, %rd18;
	ld.global.nc.f32 	%f14, [%rd21];
	neg.f32 	%f37, %f14;
	bra.uni 	BB2_4;

BB2_3:
	cvta.to.global.u64 	%rd22, %rd10;
	.loc 3 58 1
	mul.wide.s32 	%rd23, %r40, 4;
	add.s64 	%rd24, %rd22, %rd23;
	ld.global.nc.f32 	%f38, [%rd24];
	cvta.to.global.u64 	%rd25, %rd12;
	.loc 3 60 1
	add.s64 	%rd26, %rd25, %rd23;
	ld.global.nc.f32 	%f37, [%rd26];

BB2_4:
	cvta.to.global.u64 	%rd27, %rd11;
	.loc 3 59 1
	mul.wide.s32 	%rd28, %r40, 4;
	add.s64 	%rd29, %rd27, %rd28;
	cvta.to.global.u64 	%rd30, %rd9;
	.loc 3 57 1
	add.s64 	%rd31, %rd30, %rd28;
	cvta.to.global.u64 	%rd32, %rd8;
	.loc 3 56 1
	add.s64 	%rd33, %rd32, %rd28;
	cvta.to.global.u64 	%rd34, %rd7;
	.loc 3 55 1
	add.s64 	%rd35, %rd34, %rd28;
	.loc 3 71 1
	ld.global.nc.f32 	%f15, [%rd29];
	ld.global.nc.f32 	%f16, [%rd31];
	ld.global.nc.f32 	%f17, [%rd33];
	ld.global.nc.f32 	%f18, [%rd35];
	mul.f32 	%f19, %f3, %f37;
	fma.rn.f32 	%f20, %f1, %f18, %f19;
	fma.rn.f32 	%f21, %f5, %f15, %f20;
	.loc 3 47 1
	mul.wide.s32 	%rd37, %r6, 4;
	add.s64 	%rd38, %rd3, %rd37;
	.loc 3 71 1
	st.global.f32 	[%rd38], %f21;
	.loc 3 72 1
	mul.f32 	%f22, %f4, %f37;
	fma.rn.f32 	%f23, %f2, %f18, %f22;
	fma.rn.f32 	%f24, %f6, %f15, %f23;
	st.global.f32 	[%rd38+4], %f24;
	.loc 3 73 1
	mul.f32 	%f25, %f3, %f17;
	fma.rn.f32 	%f26, %f1, %f37, %f25;
	fma.rn.f32 	%f27, %f5, %f38, %f26;
	.loc 3 49 1
	add.s64 	%rd40, %rd2, %rd37;
	.loc 3 73 1
	st.global.f32 	[%rd40], %f27;
	.loc 3 74 1
	mul.f32 	%f28, %f4, %f17;
	fma.rn.f32 	%f29, %f2, %f37, %f28;
	fma.rn.f32 	%f30, %f6, %f38, %f29;
	st.global.f32 	[%rd40+4], %f30;
	.loc 3 75 1
	mul.f32 	%f31, %f3, %f38;
	fma.rn.f32 	%f32, %f1, %f15, %f31;
	fma.rn.f32 	%f33, %f5, %f16, %f32;
	.loc 3 51 1
	add.s64 	%rd42, %rd1, %rd37;
	.loc 3 75 1
	st.global.f32 	[%rd42], %f33;
	.loc 3 76 1
	mul.f32 	%f34, %f4, %f38;
	fma.rn.f32 	%f35, %f2, %f15, %f34;
	fma.rn.f32 	%f36, %f6, %f16, %f35;
	st.global.f32 	[%rd42+4], %f36;

BB2_5:
	.loc 3 77 2
	ret;
}


`
)
