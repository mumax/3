package cuda

/*
 THIS FILE IS AUTO-GENERATED BY CUDA2GO.
 EDITING IS FUTILE.
*/

import (
	"github.com/barnex/cuda5/cu"
	"unsafe"
)

var adddmi_code cu.Function

type adddmi_args struct {
	arg_Hx unsafe.Pointer
	arg_Hy unsafe.Pointer
	arg_Hz unsafe.Pointer
	arg_mx unsafe.Pointer
	arg_my unsafe.Pointer
	arg_mz unsafe.Pointer
	arg_Dx float32
	arg_Dy float32
	arg_Dz float32
	arg_A  float32
	arg_cx float32
	arg_cy float32
	arg_cz float32
	arg_N0 int
	arg_N1 int
	arg_N2 int
	argptr [16]unsafe.Pointer
}

// Wrapper for adddmi CUDA kernel, asynchronous.
func k_adddmi_async(Hx unsafe.Pointer, Hy unsafe.Pointer, Hz unsafe.Pointer, mx unsafe.Pointer, my unsafe.Pointer, mz unsafe.Pointer, Dx float32, Dy float32, Dz float32, A float32, cx float32, cy float32, cz float32, N0 int, N1 int, N2 int, cfg *config, str int) {
	if adddmi_code == 0 {
		adddmi_code = fatbinLoad(adddmi_map, "adddmi")
	}

	var _a_ adddmi_args

	_a_.arg_Hx = Hx
	_a_.argptr[0] = unsafe.Pointer(&_a_.arg_Hx)
	_a_.arg_Hy = Hy
	_a_.argptr[1] = unsafe.Pointer(&_a_.arg_Hy)
	_a_.arg_Hz = Hz
	_a_.argptr[2] = unsafe.Pointer(&_a_.arg_Hz)
	_a_.arg_mx = mx
	_a_.argptr[3] = unsafe.Pointer(&_a_.arg_mx)
	_a_.arg_my = my
	_a_.argptr[4] = unsafe.Pointer(&_a_.arg_my)
	_a_.arg_mz = mz
	_a_.argptr[5] = unsafe.Pointer(&_a_.arg_mz)
	_a_.arg_Dx = Dx
	_a_.argptr[6] = unsafe.Pointer(&_a_.arg_Dx)
	_a_.arg_Dy = Dy
	_a_.argptr[7] = unsafe.Pointer(&_a_.arg_Dy)
	_a_.arg_Dz = Dz
	_a_.argptr[8] = unsafe.Pointer(&_a_.arg_Dz)
	_a_.arg_A = A
	_a_.argptr[9] = unsafe.Pointer(&_a_.arg_A)
	_a_.arg_cx = cx
	_a_.argptr[10] = unsafe.Pointer(&_a_.arg_cx)
	_a_.arg_cy = cy
	_a_.argptr[11] = unsafe.Pointer(&_a_.arg_cy)
	_a_.arg_cz = cz
	_a_.argptr[12] = unsafe.Pointer(&_a_.arg_cz)
	_a_.arg_N0 = N0
	_a_.argptr[13] = unsafe.Pointer(&_a_.arg_N0)
	_a_.arg_N1 = N1
	_a_.argptr[14] = unsafe.Pointer(&_a_.arg_N1)
	_a_.arg_N2 = N2
	_a_.argptr[15] = unsafe.Pointer(&_a_.arg_N2)

	args := _a_.argptr[:]
	cu.LaunchKernel(adddmi_code, cfg.Grid.X, cfg.Grid.Y, cfg.Grid.Z, cfg.Block.X, cfg.Block.Y, cfg.Block.Z, 0, stream[str], args)
}

// Wrapper for adddmi CUDA kernel, synchronized.
func k_adddmi(Hx unsafe.Pointer, Hy unsafe.Pointer, Hz unsafe.Pointer, mx unsafe.Pointer, my unsafe.Pointer, mz unsafe.Pointer, Dx float32, Dy float32, Dz float32, A float32, cx float32, cy float32, cz float32, N0 int, N1 int, N2 int, cfg *config) {
	const stream = 0
	k_adddmi_async(Hx, Hy, Hz, mx, my, mz, Dx, Dy, Dz, A, cx, cy, cz, N0, N1, N2, cfg, stream)
	Sync(stream)
}

var adddmi_map = map[int]string{0: "",
	20: adddmi_ptx_20,
	30: adddmi_ptx_30,
	35: adddmi_ptx_35}

const (
	adddmi_ptx_20 = `
.version 3.1
.target sm_20
.address_size 64


.visible .entry adddmi(
	.param .u64 adddmi_param_0,
	.param .u64 adddmi_param_1,
	.param .u64 adddmi_param_2,
	.param .u64 adddmi_param_3,
	.param .u64 adddmi_param_4,
	.param .u64 adddmi_param_5,
	.param .f32 adddmi_param_6,
	.param .f32 adddmi_param_7,
	.param .f32 adddmi_param_8,
	.param .f32 adddmi_param_9,
	.param .f32 adddmi_param_10,
	.param .f32 adddmi_param_11,
	.param .f32 adddmi_param_12,
	.param .u32 adddmi_param_13,
	.param .u32 adddmi_param_14,
	.param .u32 adddmi_param_15
)
{
	.reg .pred 	%p<14>;
	.reg .s32 	%r<125>;
	.reg .f32 	%f<152>;
	.reg .s64 	%rd<55>;


	ld.param.u64 	%rd7, [adddmi_param_0];
	ld.param.u64 	%rd8, [adddmi_param_1];
	ld.param.u64 	%rd9, [adddmi_param_2];
	ld.param.u64 	%rd10, [adddmi_param_3];
	ld.param.u64 	%rd11, [adddmi_param_4];
	ld.param.u64 	%rd12, [adddmi_param_5];
	ld.param.f32 	%f47, [adddmi_param_7];
	ld.param.f32 	%f48, [adddmi_param_8];
	ld.param.f32 	%f49, [adddmi_param_9];
	ld.param.f32 	%f50, [adddmi_param_11];
	ld.param.f32 	%f51, [adddmi_param_12];
	ld.param.u32 	%r6, [adddmi_param_13];
	ld.param.u32 	%r4, [adddmi_param_14];
	ld.param.u32 	%r5, [adddmi_param_15];
	cvta.to.global.u64 	%rd1, %rd12;
	cvta.to.global.u64 	%rd2, %rd11;
	cvta.to.global.u64 	%rd3, %rd10;
	cvta.to.global.u64 	%rd4, %rd9;
	cvta.to.global.u64 	%rd5, %rd8;
	cvta.to.global.u64 	%rd6, %rd7;
	.loc 2 17 1
	mov.u32 	%r7, %ntid.z;
	mov.u32 	%r8, %ctaid.z;
	mov.u32 	%r9, %tid.z;
	mad.lo.s32 	%r1, %r7, %r8, %r9;
	.loc 2 18 1
	mov.u32 	%r10, %ntid.y;
	mov.u32 	%r11, %ctaid.y;
	mov.u32 	%r12, %tid.y;
	mad.lo.s32 	%r2, %r10, %r11, %r12;
	.loc 2 19 1
	mov.u32 	%r13, %ntid.x;
	mov.u32 	%r14, %ctaid.x;
	mov.u32 	%r15, %tid.x;
	mad.lo.s32 	%r3, %r13, %r14, %r15;
	.loc 2 21 1
	setp.ge.s32 	%p1, %r2, %r4;
	setp.ge.s32 	%p2, %r1, %r6;
	or.pred  	%p3, %p1, %p2;
	setp.ge.s32 	%p4, %r3, %r5;
	or.pred  	%p5, %p3, %p4;
	@%p5 bra 	BB0_10;

	.loc 2 25 1
	mad.lo.s32 	%r16, %r1, %r4, %r2;
	mad.lo.s32 	%r17, %r16, %r5, %r3;
	.loc 2 26 1
	mul.wide.s32 	%rd13, %r17, 4;
	add.s64 	%rd14, %rd6, %rd13;
	ld.global.f32 	%f1, [%rd14];
	add.s64 	%rd15, %rd5, %rd13;
	ld.global.f32 	%f2, [%rd15];
	add.s64 	%rd16, %rd4, %rd13;
	ld.global.f32 	%f3, [%rd16];
	.loc 2 27 1
	add.s64 	%rd17, %rd3, %rd13;
	ld.global.f32 	%f4, [%rd17];
	add.s64 	%rd18, %rd2, %rd13;
	ld.global.f32 	%f5, [%rd18];
	add.s64 	%rd19, %rd1, %rd13;
	ld.global.f32 	%f6, [%rd19];
	mov.f32 	%f9, 0f00000000;
	.loc 2 36 1
	add.s32 	%r24, %r3, 1;
	setp.ge.s32 	%p6, %r24, %r5;
	.loc 2 32 1
	mov.f32 	%f138, %f9;
	mov.f32 	%f137, %f9;
	.loc 2 36 1
	mov.f32 	%f151, %f9;
	@%p6 bra 	BB0_3;

	.loc 2 27 1
	mul.wide.s32 	%rd21, %r17, 4;
	add.s64 	%rd22, %rd3, %rd21;
	.loc 2 38 1
	ld.global.f32 	%f137, [%rd22+4];
	.loc 2 27 1
	add.s64 	%rd24, %rd2, %rd21;
	.loc 2 38 1
	ld.global.f32 	%f138, [%rd24+4];
	.loc 2 27 1
	add.s64 	%rd26, %rd1, %rd21;
	.loc 2 38 1
	ld.global.f32 	%f12, [%rd26+4];
	mov.f32 	%f151, %f12;

BB0_3:
	.loc 2 40 1
	mov.f32 	%f15, %f151;
	setp.lt.s32 	%p7, %r3, 1;
	.loc 2 32 1
	mov.f32 	%f140, %f9;
	mov.f32 	%f139, %f9;
	.loc 2 40 1
	mov.f32 	%f150, %f9;
	@%p7 bra 	BB0_5;

	.loc 2 27 1
	mul.wide.s32 	%rd28, %r17, 4;
	add.s64 	%rd29, %rd3, %rd28;
	.loc 2 42 1
	ld.global.f32 	%f139, [%rd29+-4];
	.loc 2 27 1
	add.s64 	%rd31, %rd2, %rd28;
	.loc 2 42 1
	ld.global.f32 	%f140, [%rd31+-4];
	.loc 2 27 1
	add.s64 	%rd33, %rd1, %rd28;
	.loc 2 42 1
	ld.global.f32 	%f150, [%rd33+-4];

BB0_5:
	.loc 2 46 1
	add.f32 	%f25, %f49, %f49;
	.loc 3 2399 3
	div.rn.f32 	%f52, %f48, %f25;
	.loc 2 47 1
	mul.f32 	%f53, %f138, %f138;
	fma.rn.f32 	%f54, %f137, %f137, %f53;
	fma.rn.f32 	%f55, %f15, %f15, %f54;
	.loc 4 991 5
	sqrt.rn.f32 	%f56, %f55;
	.loc 2 47 1
	setp.eq.f32 	%p8, %f56, 0f00000000;
	.loc 2 48 1
	mul.f32 	%f57, %f52, %f51;
	mul.f32 	%f58, %f57, %f6;
	fma.rn.f32 	%f59, %f57, %f6, %f4;
	.loc 2 50 1
	mul.f32 	%f60, %f57, %f4;
	sub.f32 	%f61, %f6, %f60;
	.loc 2 47 1
	selp.f32 	%f62, %f59, %f137, %p8;
	selp.f32 	%f63, %f5, %f138, %p8;
	selp.f32 	%f64, %f61, %f15, %p8;
	.loc 2 52 1
	mul.f32 	%f65, %f140, %f140;
	fma.rn.f32 	%f66, %f139, %f139, %f65;
	fma.rn.f32 	%f67, %f150, %f150, %f66;
	.loc 4 991 5
	sqrt.rn.f32 	%f68, %f67;
	.loc 2 52 1
	setp.eq.f32 	%p9, %f68, 0f00000000;
	.loc 2 53 1
	sub.f32 	%f69, %f4, %f58;
	.loc 2 55 1
	fma.rn.f32 	%f70, %f57, %f4, %f6;
	.loc 2 52 1
	selp.f32 	%f71, %f69, %f139, %p9;
	selp.f32 	%f72, %f5, %f140, %p9;
	selp.f32 	%f73, %f70, %f150, %p9;
	.loc 2 58 1
	mul.f32 	%f74, %f51, %f51;
	.loc 3 2399 3
	div.rn.f32 	%f75, %f25, %f74;
	.loc 2 58 1
	sub.f32 	%f76, %f62, %f4;
	sub.f32 	%f77, %f63, %f5;
	sub.f32 	%f78, %f64, %f6;
	sub.f32 	%f79, %f71, %f4;
	sub.f32 	%f80, %f72, %f5;
	sub.f32 	%f81, %f73, %f6;
	add.f32 	%f82, %f76, %f79;
	add.f32 	%f83, %f77, %f80;
	add.f32 	%f84, %f78, %f81;
	.loc 2 58 1
	fma.rn.f32 	%f85, %f75, %f82, %f1;
	fma.rn.f32 	%f26, %f75, %f83, %f2;
	fma.rn.f32 	%f86, %f75, %f84, %f3;
	.loc 2 59 1
	sub.f32 	%f87, %f64, %f73;
	mul.f32 	%f88, %f87, %f48;
	.loc 3 2399 3
	div.rn.f32 	%f89, %f88, %f51;
	.loc 2 59 1
	sub.f32 	%f27, %f85, %f89;
	.loc 2 60 1
	sub.f32 	%f90, %f62, %f71;
	mul.f32 	%f91, %f90, %f48;
	.loc 3 2399 3
	div.rn.f32 	%f92, %f91, %f51;
	.loc 2 60 1
	add.f32 	%f28, %f86, %f92;
	.loc 2 69 1
	add.s32 	%r67, %r2, 1;
	setp.ge.s32 	%p10, %r67, %r4;
	.loc 2 32 1
	mov.f32 	%f142, %f9;
	mov.f32 	%f141, %f9;
	.loc 2 69 1
	mov.f32 	%f149, %f9;
	@%p10 bra 	BB0_7;

	.loc 2 70 1
	add.s32 	%r77, %r16, 1;
	mad.lo.s32 	%r82, %r77, %r5, %r3;
	.loc 2 71 1
	mul.wide.s32 	%rd35, %r82, 4;
	add.s64 	%rd36, %rd3, %rd35;
	ld.global.f32 	%f141, [%rd36];
	add.s64 	%rd38, %rd2, %rd35;
	ld.global.f32 	%f142, [%rd38];
	add.s64 	%rd40, %rd1, %rd35;
	ld.global.f32 	%f149, [%rd40];

BB0_7:
	.loc 2 73 1
	setp.lt.s32 	%p11, %r2, 1;
	.loc 2 32 1
	mov.f32 	%f144, %f9;
	mov.f32 	%f143, %f9;
	.loc 2 73 1
	mov.f32 	%f148, %f9;
	@%p11 bra 	BB0_9;

	.loc 2 74 1
	add.s32 	%r99, %r16, -1;
	mad.lo.s32 	%r104, %r99, %r5, %r3;
	.loc 2 75 1
	mul.wide.s32 	%rd42, %r104, 4;
	add.s64 	%rd43, %rd3, %rd42;
	ld.global.f32 	%f143, [%rd43];
	add.s64 	%rd45, %rd2, %rd42;
	ld.global.f32 	%f144, [%rd45];
	add.s64 	%rd47, %rd1, %rd42;
	ld.global.f32 	%f148, [%rd47];

BB0_9:
	.loc 2 79 1
	mul.f32 	%f93, %f142, %f142;
	fma.rn.f32 	%f94, %f141, %f141, %f93;
	fma.rn.f32 	%f95, %f149, %f149, %f94;
	.loc 4 991 5
	sqrt.rn.f32 	%f96, %f95;
	.loc 2 79 1
	setp.eq.f32 	%p12, %f96, 0f00000000;
	.loc 3 2399 3
	div.rn.f32 	%f97, %f47, %f25;
	.loc 2 80 1
	mul.f32 	%f98, %f97, %f50;
	mul.f32 	%f99, %f98, %f5;
	sub.f32 	%f100, %f4, %f99;
	.loc 2 81 1
	mul.f32 	%f101, %f98, %f4;
	fma.rn.f32 	%f102, %f98, %f4, %f5;
	.loc 2 79 1
	selp.f32 	%f103, %f100, %f141, %p12;
	selp.f32 	%f104, %f102, %f142, %p12;
	selp.f32 	%f105, %f6, %f149, %p12;
	.loc 2 84 1
	mul.f32 	%f106, %f144, %f144;
	fma.rn.f32 	%f107, %f143, %f143, %f106;
	fma.rn.f32 	%f108, %f148, %f148, %f107;
	.loc 4 991 5
	sqrt.rn.f32 	%f109, %f108;
	.loc 2 84 1
	setp.eq.f32 	%p13, %f109, 0f00000000;
	.loc 2 85 1
	fma.rn.f32 	%f110, %f98, %f5, %f4;
	.loc 2 86 1
	sub.f32 	%f111, %f5, %f101;
	.loc 2 84 1
	selp.f32 	%f112, %f110, %f143, %p13;
	selp.f32 	%f113, %f111, %f144, %p13;
	selp.f32 	%f114, %f6, %f148, %p13;
	.loc 2 90 1
	mul.f32 	%f115, %f50, %f50;
	.loc 3 2399 3
	div.rn.f32 	%f116, %f25, %f115;
	.loc 2 90 1
	sub.f32 	%f117, %f103, %f4;
	sub.f32 	%f118, %f104, %f5;
	sub.f32 	%f119, %f105, %f6;
	sub.f32 	%f120, %f112, %f4;
	sub.f32 	%f121, %f113, %f5;
	sub.f32 	%f122, %f114, %f6;
	add.f32 	%f123, %f117, %f120;
	add.f32 	%f124, %f118, %f121;
	add.f32 	%f125, %f119, %f122;
	.loc 2 90 1
	fma.rn.f32 	%f126, %f116, %f123, %f27;
	fma.rn.f32 	%f127, %f116, %f124, %f26;
	fma.rn.f32 	%f128, %f116, %f125, %f28;
	.loc 2 91 1
	sub.f32 	%f129, %f104, %f113;
	mul.f32 	%f130, %f129, %f47;
	.loc 3 2399 3
	div.rn.f32 	%f131, %f130, %f50;
	.loc 2 91 1
	add.f32 	%f132, %f126, %f131;
	.loc 2 92 1
	sub.f32 	%f133, %f103, %f112;
	mul.f32 	%f134, %f133, %f47;
	.loc 3 2399 3
	div.rn.f32 	%f135, %f134, %f50;
	.loc 2 92 1
	sub.f32 	%f136, %f127, %f135;
	.loc 2 26 1
	mul.wide.s32 	%rd49, %r17, 4;
	add.s64 	%rd50, %rd6, %rd49;
	.loc 2 96 1
	st.global.f32 	[%rd50], %f132;
	.loc 2 26 1
	add.s64 	%rd52, %rd5, %rd49;
	.loc 2 97 1
	st.global.f32 	[%rd52], %f136;
	.loc 2 26 1
	add.s64 	%rd54, %rd4, %rd49;
	.loc 2 98 1
	st.global.f32 	[%rd54], %f128;

BB0_10:
	.loc 2 99 2
	ret;
}


`
	adddmi_ptx_30 = `
.version 3.1
.target sm_30
.address_size 64


.visible .entry adddmi(
	.param .u64 adddmi_param_0,
	.param .u64 adddmi_param_1,
	.param .u64 adddmi_param_2,
	.param .u64 adddmi_param_3,
	.param .u64 adddmi_param_4,
	.param .u64 adddmi_param_5,
	.param .f32 adddmi_param_6,
	.param .f32 adddmi_param_7,
	.param .f32 adddmi_param_8,
	.param .f32 adddmi_param_9,
	.param .f32 adddmi_param_10,
	.param .f32 adddmi_param_11,
	.param .f32 adddmi_param_12,
	.param .u32 adddmi_param_13,
	.param .u32 adddmi_param_14,
	.param .u32 adddmi_param_15
)
{
	.reg .pred 	%p<14>;
	.reg .s32 	%r<48>;
	.reg .f32 	%f<152>;
	.reg .s64 	%rd<56>;


	ld.param.u64 	%rd8, [adddmi_param_0];
	ld.param.u64 	%rd9, [adddmi_param_1];
	ld.param.u64 	%rd10, [adddmi_param_2];
	ld.param.u64 	%rd11, [adddmi_param_3];
	ld.param.u64 	%rd12, [adddmi_param_4];
	ld.param.u64 	%rd13, [adddmi_param_5];
	ld.param.f32 	%f47, [adddmi_param_7];
	ld.param.f32 	%f48, [adddmi_param_8];
	ld.param.f32 	%f49, [adddmi_param_9];
	ld.param.f32 	%f50, [adddmi_param_11];
	ld.param.f32 	%f51, [adddmi_param_12];
	ld.param.u32 	%r7, [adddmi_param_13];
	ld.param.u32 	%r5, [adddmi_param_14];
	ld.param.u32 	%r6, [adddmi_param_15];
	cvta.to.global.u64 	%rd1, %rd13;
	cvta.to.global.u64 	%rd2, %rd12;
	cvta.to.global.u64 	%rd3, %rd11;
	cvta.to.global.u64 	%rd4, %rd10;
	cvta.to.global.u64 	%rd5, %rd9;
	cvta.to.global.u64 	%rd6, %rd8;
	.loc 2 17 1
	mov.u32 	%r8, %ntid.z;
	mov.u32 	%r9, %ctaid.z;
	mov.u32 	%r10, %tid.z;
	mad.lo.s32 	%r1, %r8, %r9, %r10;
	.loc 2 18 1
	mov.u32 	%r11, %ntid.y;
	mov.u32 	%r12, %ctaid.y;
	mov.u32 	%r13, %tid.y;
	mad.lo.s32 	%r2, %r11, %r12, %r13;
	.loc 2 19 1
	mov.u32 	%r14, %ntid.x;
	mov.u32 	%r15, %ctaid.x;
	mov.u32 	%r16, %tid.x;
	mad.lo.s32 	%r3, %r14, %r15, %r16;
	.loc 2 21 1
	setp.ge.s32 	%p1, %r2, %r5;
	setp.ge.s32 	%p2, %r1, %r7;
	or.pred  	%p3, %p1, %p2;
	setp.ge.s32 	%p4, %r3, %r6;
	or.pred  	%p5, %p3, %p4;
	@%p5 bra 	BB0_10;

	.loc 2 25 1
	mul.lo.s32 	%r4, %r1, %r5;
	mad.lo.s32 	%r17, %r1, %r5, %r2;
	mad.lo.s32 	%r18, %r17, %r6, %r3;
	.loc 2 26 1
	cvt.s64.s32 	%rd7, %r18;
	mul.wide.s32 	%rd14, %r18, 4;
	add.s64 	%rd15, %rd6, %rd14;
	ld.global.f32 	%f1, [%rd15];
	add.s64 	%rd16, %rd5, %rd14;
	ld.global.f32 	%f2, [%rd16];
	add.s64 	%rd17, %rd4, %rd14;
	ld.global.f32 	%f3, [%rd17];
	.loc 2 27 1
	add.s64 	%rd18, %rd3, %rd14;
	ld.global.f32 	%f4, [%rd18];
	add.s64 	%rd19, %rd2, %rd14;
	ld.global.f32 	%f5, [%rd19];
	add.s64 	%rd20, %rd1, %rd14;
	ld.global.f32 	%f6, [%rd20];
	mov.f32 	%f9, 0f00000000;
	.loc 2 36 1
	add.s32 	%r25, %r3, 1;
	setp.ge.s32 	%p6, %r25, %r6;
	.loc 2 32 1
	mov.f32 	%f138, %f9;
	mov.f32 	%f137, %f9;
	.loc 2 36 1
	mov.f32 	%f151, %f9;
	@%p6 bra 	BB0_3;

	.loc 2 27 1
	shl.b64 	%rd22, %rd7, 2;
	add.s64 	%rd23, %rd3, %rd22;
	.loc 2 38 1
	ld.global.f32 	%f137, [%rd23+4];
	.loc 2 27 1
	add.s64 	%rd25, %rd2, %rd22;
	.loc 2 38 1
	ld.global.f32 	%f138, [%rd25+4];
	.loc 2 27 1
	add.s64 	%rd27, %rd1, %rd22;
	.loc 2 38 1
	ld.global.f32 	%f12, [%rd27+4];
	mov.f32 	%f151, %f12;

BB0_3:
	.loc 2 40 1
	mov.f32 	%f15, %f151;
	setp.lt.s32 	%p7, %r3, 1;
	.loc 2 32 1
	mov.f32 	%f140, %f9;
	mov.f32 	%f139, %f9;
	.loc 2 40 1
	mov.f32 	%f150, %f9;
	@%p7 bra 	BB0_5;

	.loc 2 27 1
	shl.b64 	%rd29, %rd7, 2;
	add.s64 	%rd30, %rd3, %rd29;
	.loc 2 42 1
	ld.global.f32 	%f139, [%rd30+-4];
	.loc 2 27 1
	add.s64 	%rd32, %rd2, %rd29;
	.loc 2 42 1
	ld.global.f32 	%f140, [%rd32+-4];
	.loc 2 27 1
	add.s64 	%rd34, %rd1, %rd29;
	.loc 2 42 1
	ld.global.f32 	%f150, [%rd34+-4];

BB0_5:
	.loc 2 46 1
	add.f32 	%f25, %f49, %f49;
	.loc 3 2399 3
	div.rn.f32 	%f52, %f48, %f25;
	.loc 2 47 1
	mul.f32 	%f53, %f138, %f138;
	fma.rn.f32 	%f54, %f137, %f137, %f53;
	fma.rn.f32 	%f55, %f15, %f15, %f54;
	.loc 4 991 5
	sqrt.rn.f32 	%f56, %f55;
	.loc 2 47 1
	setp.eq.f32 	%p8, %f56, 0f00000000;
	.loc 2 48 1
	mul.f32 	%f57, %f52, %f51;
	mul.f32 	%f58, %f57, %f6;
	fma.rn.f32 	%f59, %f57, %f6, %f4;
	.loc 2 50 1
	mul.f32 	%f60, %f57, %f4;
	sub.f32 	%f61, %f6, %f60;
	.loc 2 47 1
	selp.f32 	%f62, %f59, %f137, %p8;
	selp.f32 	%f63, %f5, %f138, %p8;
	selp.f32 	%f64, %f61, %f15, %p8;
	.loc 2 52 1
	mul.f32 	%f65, %f140, %f140;
	fma.rn.f32 	%f66, %f139, %f139, %f65;
	fma.rn.f32 	%f67, %f150, %f150, %f66;
	.loc 4 991 5
	sqrt.rn.f32 	%f68, %f67;
	.loc 2 52 1
	setp.eq.f32 	%p9, %f68, 0f00000000;
	.loc 2 53 1
	sub.f32 	%f69, %f4, %f58;
	.loc 2 55 1
	fma.rn.f32 	%f70, %f57, %f4, %f6;
	.loc 2 52 1
	selp.f32 	%f71, %f69, %f139, %p9;
	selp.f32 	%f72, %f5, %f140, %p9;
	selp.f32 	%f73, %f70, %f150, %p9;
	.loc 2 58 1
	mul.f32 	%f74, %f51, %f51;
	.loc 3 2399 3
	div.rn.f32 	%f75, %f25, %f74;
	.loc 2 58 1
	sub.f32 	%f76, %f62, %f4;
	sub.f32 	%f77, %f63, %f5;
	sub.f32 	%f78, %f64, %f6;
	sub.f32 	%f79, %f71, %f4;
	sub.f32 	%f80, %f72, %f5;
	sub.f32 	%f81, %f73, %f6;
	add.f32 	%f82, %f76, %f79;
	add.f32 	%f83, %f77, %f80;
	add.f32 	%f84, %f78, %f81;
	.loc 2 58 1
	fma.rn.f32 	%f85, %f75, %f82, %f1;
	fma.rn.f32 	%f26, %f75, %f83, %f2;
	fma.rn.f32 	%f86, %f75, %f84, %f3;
	.loc 2 59 1
	sub.f32 	%f87, %f64, %f73;
	mul.f32 	%f88, %f87, %f48;
	.loc 3 2399 3
	div.rn.f32 	%f89, %f88, %f51;
	.loc 2 59 1
	sub.f32 	%f27, %f85, %f89;
	.loc 2 60 1
	sub.f32 	%f90, %f62, %f71;
	mul.f32 	%f91, %f90, %f48;
	.loc 3 2399 3
	div.rn.f32 	%f92, %f91, %f51;
	.loc 2 60 1
	add.f32 	%f28, %f86, %f92;
	.loc 2 69 1
	add.s32 	%r32, %r2, 1;
	setp.ge.s32 	%p10, %r32, %r5;
	.loc 2 32 1
	mov.f32 	%f142, %f9;
	mov.f32 	%f141, %f9;
	.loc 2 69 1
	mov.f32 	%f149, %f9;
	@%p10 bra 	BB0_7;

	.loc 2 70 1
	add.s32 	%r33, %r4, %r2;
	add.s32 	%r34, %r33, 1;
	mad.lo.s32 	%r35, %r34, %r6, %r3;
	.loc 2 71 1
	mul.wide.s32 	%rd36, %r35, 4;
	add.s64 	%rd37, %rd3, %rd36;
	ld.global.f32 	%f141, [%rd37];
	add.s64 	%rd39, %rd2, %rd36;
	ld.global.f32 	%f142, [%rd39];
	add.s64 	%rd41, %rd1, %rd36;
	ld.global.f32 	%f149, [%rd41];

BB0_7:
	.loc 2 73 1
	setp.lt.s32 	%p11, %r2, 1;
	.loc 2 32 1
	mov.f32 	%f144, %f9;
	mov.f32 	%f143, %f9;
	.loc 2 73 1
	mov.f32 	%f148, %f9;
	@%p11 bra 	BB0_9;

	.loc 2 74 1
	add.s32 	%r39, %r4, %r2;
	add.s32 	%r40, %r39, -1;
	mad.lo.s32 	%r41, %r40, %r6, %r3;
	.loc 2 75 1
	mul.wide.s32 	%rd43, %r41, 4;
	add.s64 	%rd44, %rd3, %rd43;
	ld.global.f32 	%f143, [%rd44];
	add.s64 	%rd46, %rd2, %rd43;
	ld.global.f32 	%f144, [%rd46];
	add.s64 	%rd48, %rd1, %rd43;
	ld.global.f32 	%f148, [%rd48];

BB0_9:
	.loc 2 79 1
	mul.f32 	%f93, %f142, %f142;
	fma.rn.f32 	%f94, %f141, %f141, %f93;
	fma.rn.f32 	%f95, %f149, %f149, %f94;
	.loc 4 991 5
	sqrt.rn.f32 	%f96, %f95;
	.loc 2 79 1
	setp.eq.f32 	%p12, %f96, 0f00000000;
	.loc 3 2399 3
	div.rn.f32 	%f97, %f47, %f25;
	.loc 2 80 1
	mul.f32 	%f98, %f97, %f50;
	mul.f32 	%f99, %f98, %f5;
	sub.f32 	%f100, %f4, %f99;
	.loc 2 81 1
	mul.f32 	%f101, %f98, %f4;
	fma.rn.f32 	%f102, %f98, %f4, %f5;
	.loc 2 79 1
	selp.f32 	%f103, %f100, %f141, %p12;
	selp.f32 	%f104, %f102, %f142, %p12;
	selp.f32 	%f105, %f6, %f149, %p12;
	.loc 2 84 1
	mul.f32 	%f106, %f144, %f144;
	fma.rn.f32 	%f107, %f143, %f143, %f106;
	fma.rn.f32 	%f108, %f148, %f148, %f107;
	.loc 4 991 5
	sqrt.rn.f32 	%f109, %f108;
	.loc 2 84 1
	setp.eq.f32 	%p13, %f109, 0f00000000;
	.loc 2 85 1
	fma.rn.f32 	%f110, %f98, %f5, %f4;
	.loc 2 86 1
	sub.f32 	%f111, %f5, %f101;
	.loc 2 84 1
	selp.f32 	%f112, %f110, %f143, %p13;
	selp.f32 	%f113, %f111, %f144, %p13;
	selp.f32 	%f114, %f6, %f148, %p13;
	.loc 2 90 1
	mul.f32 	%f115, %f50, %f50;
	.loc 3 2399 3
	div.rn.f32 	%f116, %f25, %f115;
	.loc 2 90 1
	sub.f32 	%f117, %f103, %f4;
	sub.f32 	%f118, %f104, %f5;
	sub.f32 	%f119, %f105, %f6;
	sub.f32 	%f120, %f112, %f4;
	sub.f32 	%f121, %f113, %f5;
	sub.f32 	%f122, %f114, %f6;
	add.f32 	%f123, %f117, %f120;
	add.f32 	%f124, %f118, %f121;
	add.f32 	%f125, %f119, %f122;
	.loc 2 90 1
	fma.rn.f32 	%f126, %f116, %f123, %f27;
	fma.rn.f32 	%f127, %f116, %f124, %f26;
	fma.rn.f32 	%f128, %f116, %f125, %f28;
	.loc 2 91 1
	sub.f32 	%f129, %f104, %f113;
	mul.f32 	%f130, %f129, %f47;
	.loc 3 2399 3
	div.rn.f32 	%f131, %f130, %f50;
	.loc 2 91 1
	add.f32 	%f132, %f126, %f131;
	.loc 2 92 1
	sub.f32 	%f133, %f103, %f112;
	mul.f32 	%f134, %f133, %f47;
	.loc 3 2399 3
	div.rn.f32 	%f135, %f134, %f50;
	.loc 2 92 1
	sub.f32 	%f136, %f127, %f135;
	.loc 2 26 1
	shl.b64 	%rd50, %rd7, 2;
	add.s64 	%rd51, %rd6, %rd50;
	.loc 2 96 1
	st.global.f32 	[%rd51], %f132;
	.loc 2 26 1
	add.s64 	%rd53, %rd5, %rd50;
	.loc 2 97 1
	st.global.f32 	[%rd53], %f136;
	.loc 2 26 1
	add.s64 	%rd55, %rd4, %rd50;
	.loc 2 98 1
	st.global.f32 	[%rd55], %f128;

BB0_10:
	.loc 2 99 2
	ret;
}


`
	adddmi_ptx_35 = `
.version 3.1
.target sm_35
.address_size 64


.weak .func  (.param .b32 func_retval0) cudaMalloc(
	.param .b64 cudaMalloc_param_0,
	.param .b64 cudaMalloc_param_1
)
{
	.reg .s32 	%r<2>;


	mov.u32 	%r1, 30;
	st.param.b32	[func_retval0+0], %r1;
	.loc 2 66 3
	ret;
}

.weak .func  (.param .b32 func_retval0) cudaFuncGetAttributes(
	.param .b64 cudaFuncGetAttributes_param_0,
	.param .b64 cudaFuncGetAttributes_param_1
)
{
	.reg .s32 	%r<2>;


	mov.u32 	%r1, 30;
	st.param.b32	[func_retval0+0], %r1;
	.loc 2 71 3
	ret;
}

.visible .entry adddmi(
	.param .u64 adddmi_param_0,
	.param .u64 adddmi_param_1,
	.param .u64 adddmi_param_2,
	.param .u64 adddmi_param_3,
	.param .u64 adddmi_param_4,
	.param .u64 adddmi_param_5,
	.param .f32 adddmi_param_6,
	.param .f32 adddmi_param_7,
	.param .f32 adddmi_param_8,
	.param .f32 adddmi_param_9,
	.param .f32 adddmi_param_10,
	.param .f32 adddmi_param_11,
	.param .f32 adddmi_param_12,
	.param .u32 adddmi_param_13,
	.param .u32 adddmi_param_14,
	.param .u32 adddmi_param_15
)
{
	.reg .pred 	%p<14>;
	.reg .s32 	%r<38>;
	.reg .f32 	%f<152>;
	.reg .s64 	%rd<56>;


	ld.param.u64 	%rd8, [adddmi_param_0];
	ld.param.u64 	%rd9, [adddmi_param_1];
	ld.param.u64 	%rd10, [adddmi_param_2];
	ld.param.u64 	%rd11, [adddmi_param_3];
	ld.param.u64 	%rd12, [adddmi_param_4];
	ld.param.u64 	%rd13, [adddmi_param_5];
	ld.param.f32 	%f47, [adddmi_param_7];
	ld.param.f32 	%f48, [adddmi_param_8];
	ld.param.f32 	%f49, [adddmi_param_9];
	ld.param.f32 	%f50, [adddmi_param_11];
	ld.param.f32 	%f51, [adddmi_param_12];
	ld.param.u32 	%r8, [adddmi_param_13];
	ld.param.u32 	%r6, [adddmi_param_14];
	ld.param.u32 	%r7, [adddmi_param_15];
	cvta.to.global.u64 	%rd1, %rd13;
	cvta.to.global.u64 	%rd2, %rd12;
	cvta.to.global.u64 	%rd3, %rd11;
	cvta.to.global.u64 	%rd4, %rd10;
	cvta.to.global.u64 	%rd5, %rd9;
	cvta.to.global.u64 	%rd6, %rd8;
	.loc 3 17 1
	mov.u32 	%r9, %ntid.z;
	mov.u32 	%r10, %ctaid.z;
	mov.u32 	%r11, %tid.z;
	mad.lo.s32 	%r1, %r9, %r10, %r11;
	.loc 3 18 1
	mov.u32 	%r12, %ntid.y;
	mov.u32 	%r13, %ctaid.y;
	mov.u32 	%r14, %tid.y;
	mad.lo.s32 	%r2, %r12, %r13, %r14;
	.loc 3 19 1
	mov.u32 	%r15, %ntid.x;
	mov.u32 	%r16, %ctaid.x;
	mov.u32 	%r17, %tid.x;
	mad.lo.s32 	%r3, %r15, %r16, %r17;
	.loc 3 21 1
	setp.ge.s32 	%p1, %r2, %r6;
	setp.ge.s32 	%p2, %r1, %r8;
	or.pred  	%p3, %p1, %p2;
	setp.ge.s32 	%p4, %r3, %r7;
	or.pred  	%p5, %p3, %p4;
	@%p5 bra 	BB2_10;

	.loc 3 25 1
	mul.lo.s32 	%r4, %r1, %r6;
	mad.lo.s32 	%r18, %r1, %r6, %r2;
	mul.lo.s32 	%r5, %r18, %r7;
	mad.lo.s32 	%r19, %r18, %r7, %r3;
	.loc 3 26 1
	cvt.s64.s32 	%rd7, %r19;
	mul.wide.s32 	%rd14, %r19, 4;
	add.s64 	%rd15, %rd6, %rd14;
	ld.global.f32 	%f1, [%rd15];
	add.s64 	%rd16, %rd5, %rd14;
	ld.global.f32 	%f2, [%rd16];
	add.s64 	%rd17, %rd4, %rd14;
	ld.global.f32 	%f3, [%rd17];
	.loc 3 27 1
	add.s64 	%rd18, %rd3, %rd14;
	ld.global.nc.f32 	%f4, [%rd18];
	add.s64 	%rd19, %rd2, %rd14;
	ld.global.nc.f32 	%f5, [%rd19];
	add.s64 	%rd20, %rd1, %rd14;
	ld.global.nc.f32 	%f6, [%rd20];
	mov.f32 	%f9, 0f00000000;
	.loc 3 36 1
	add.s32 	%r23, %r3, 1;
	setp.ge.s32 	%p6, %r23, %r7;
	.loc 3 32 1
	mov.f32 	%f138, %f9;
	mov.f32 	%f137, %f9;
	.loc 3 36 1
	mov.f32 	%f151, %f9;
	@%p6 bra 	BB2_3;

	.loc 3 37 1
	add.s32 	%r24, %r5, %r3;
	add.s32 	%r25, %r24, 1;
	.loc 3 38 1
	mul.wide.s32 	%rd22, %r25, 4;
	add.s64 	%rd23, %rd3, %rd22;
	ld.global.nc.f32 	%f137, [%rd23];
	add.s64 	%rd25, %rd2, %rd22;
	ld.global.nc.f32 	%f138, [%rd25];
	add.s64 	%rd27, %rd1, %rd22;
	ld.global.nc.f32 	%f12, [%rd27];
	mov.f32 	%f151, %f12;

BB2_3:
	.loc 3 40 1
	mov.f32 	%f15, %f151;
	setp.lt.s32 	%p7, %r3, 1;
	.loc 3 32 1
	mov.f32 	%f140, %f9;
	mov.f32 	%f139, %f9;
	.loc 3 40 1
	mov.f32 	%f150, %f9;
	@%p7 bra 	BB2_5;

	.loc 3 41 1
	add.s32 	%r26, %r5, %r3;
	add.s32 	%r27, %r26, -1;
	.loc 3 42 1
	mul.wide.s32 	%rd29, %r27, 4;
	add.s64 	%rd30, %rd3, %rd29;
	ld.global.nc.f32 	%f139, [%rd30];
	add.s64 	%rd32, %rd2, %rd29;
	ld.global.nc.f32 	%f140, [%rd32];
	add.s64 	%rd34, %rd1, %rd29;
	ld.global.nc.f32 	%f150, [%rd34];

BB2_5:
	.loc 3 46 1
	add.f32 	%f25, %f49, %f49;
	.loc 4 2399 3
	div.rn.f32 	%f52, %f48, %f25;
	.loc 3 47 1
	mul.f32 	%f53, %f138, %f138;
	fma.rn.f32 	%f54, %f137, %f137, %f53;
	fma.rn.f32 	%f55, %f15, %f15, %f54;
	.loc 5 991 5
	sqrt.rn.f32 	%f56, %f55;
	.loc 3 47 1
	setp.eq.f32 	%p8, %f56, 0f00000000;
	.loc 3 48 1
	mul.f32 	%f57, %f52, %f51;
	mul.f32 	%f58, %f57, %f6;
	fma.rn.f32 	%f59, %f57, %f6, %f4;
	.loc 3 50 1
	mul.f32 	%f60, %f57, %f4;
	sub.f32 	%f61, %f6, %f60;
	.loc 3 47 1
	selp.f32 	%f62, %f59, %f137, %p8;
	selp.f32 	%f63, %f5, %f138, %p8;
	selp.f32 	%f64, %f61, %f15, %p8;
	.loc 3 52 1
	mul.f32 	%f65, %f140, %f140;
	fma.rn.f32 	%f66, %f139, %f139, %f65;
	fma.rn.f32 	%f67, %f150, %f150, %f66;
	.loc 5 991 5
	sqrt.rn.f32 	%f68, %f67;
	.loc 3 52 1
	setp.eq.f32 	%p9, %f68, 0f00000000;
	.loc 3 53 1
	sub.f32 	%f69, %f4, %f58;
	.loc 3 55 1
	fma.rn.f32 	%f70, %f57, %f4, %f6;
	.loc 3 52 1
	selp.f32 	%f71, %f69, %f139, %p9;
	selp.f32 	%f72, %f5, %f140, %p9;
	selp.f32 	%f73, %f70, %f150, %p9;
	.loc 3 58 1
	mul.f32 	%f74, %f51, %f51;
	.loc 4 2399 3
	div.rn.f32 	%f75, %f25, %f74;
	.loc 3 58 1
	sub.f32 	%f76, %f62, %f4;
	sub.f32 	%f77, %f63, %f5;
	sub.f32 	%f78, %f64, %f6;
	sub.f32 	%f79, %f71, %f4;
	sub.f32 	%f80, %f72, %f5;
	sub.f32 	%f81, %f73, %f6;
	add.f32 	%f82, %f76, %f79;
	add.f32 	%f83, %f77, %f80;
	add.f32 	%f84, %f78, %f81;
	.loc 3 58 1
	fma.rn.f32 	%f85, %f75, %f82, %f1;
	fma.rn.f32 	%f26, %f75, %f83, %f2;
	fma.rn.f32 	%f86, %f75, %f84, %f3;
	.loc 3 59 1
	sub.f32 	%f87, %f64, %f73;
	mul.f32 	%f88, %f87, %f48;
	.loc 4 2399 3
	div.rn.f32 	%f89, %f88, %f51;
	.loc 3 59 1
	sub.f32 	%f27, %f85, %f89;
	.loc 3 60 1
	sub.f32 	%f90, %f62, %f71;
	mul.f32 	%f91, %f90, %f48;
	.loc 4 2399 3
	div.rn.f32 	%f92, %f91, %f51;
	.loc 3 60 1
	add.f32 	%f28, %f86, %f92;
	.loc 3 69 1
	add.s32 	%r28, %r2, 1;
	setp.ge.s32 	%p10, %r28, %r6;
	.loc 3 32 1
	mov.f32 	%f142, %f9;
	mov.f32 	%f141, %f9;
	.loc 3 69 1
	mov.f32 	%f149, %f9;
	@%p10 bra 	BB2_7;

	.loc 3 70 1
	add.s32 	%r29, %r4, %r2;
	add.s32 	%r30, %r29, 1;
	mad.lo.s32 	%r31, %r30, %r7, %r3;
	.loc 3 71 1
	mul.wide.s32 	%rd36, %r31, 4;
	add.s64 	%rd37, %rd3, %rd36;
	ld.global.nc.f32 	%f141, [%rd37];
	add.s64 	%rd39, %rd2, %rd36;
	ld.global.nc.f32 	%f142, [%rd39];
	add.s64 	%rd41, %rd1, %rd36;
	ld.global.nc.f32 	%f149, [%rd41];

BB2_7:
	.loc 3 73 1
	setp.lt.s32 	%p11, %r2, 1;
	.loc 3 32 1
	mov.f32 	%f144, %f9;
	mov.f32 	%f143, %f9;
	.loc 3 73 1
	mov.f32 	%f148, %f9;
	@%p11 bra 	BB2_9;

	.loc 3 74 1
	add.s32 	%r32, %r4, %r2;
	add.s32 	%r33, %r32, -1;
	mad.lo.s32 	%r34, %r33, %r7, %r3;
	.loc 3 75 1
	mul.wide.s32 	%rd43, %r34, 4;
	add.s64 	%rd44, %rd3, %rd43;
	ld.global.nc.f32 	%f143, [%rd44];
	add.s64 	%rd46, %rd2, %rd43;
	ld.global.nc.f32 	%f144, [%rd46];
	add.s64 	%rd48, %rd1, %rd43;
	ld.global.nc.f32 	%f148, [%rd48];

BB2_9:
	.loc 3 79 1
	mul.f32 	%f93, %f142, %f142;
	fma.rn.f32 	%f94, %f141, %f141, %f93;
	fma.rn.f32 	%f95, %f149, %f149, %f94;
	.loc 5 991 5
	sqrt.rn.f32 	%f96, %f95;
	.loc 3 79 1
	setp.eq.f32 	%p12, %f96, 0f00000000;
	.loc 4 2399 3
	div.rn.f32 	%f97, %f47, %f25;
	.loc 3 80 1
	mul.f32 	%f98, %f97, %f50;
	mul.f32 	%f99, %f98, %f5;
	sub.f32 	%f100, %f4, %f99;
	.loc 3 81 1
	mul.f32 	%f101, %f98, %f4;
	fma.rn.f32 	%f102, %f98, %f4, %f5;
	.loc 3 79 1
	selp.f32 	%f103, %f100, %f141, %p12;
	selp.f32 	%f104, %f102, %f142, %p12;
	selp.f32 	%f105, %f6, %f149, %p12;
	.loc 3 84 1
	mul.f32 	%f106, %f144, %f144;
	fma.rn.f32 	%f107, %f143, %f143, %f106;
	fma.rn.f32 	%f108, %f148, %f148, %f107;
	.loc 5 991 5
	sqrt.rn.f32 	%f109, %f108;
	.loc 3 84 1
	setp.eq.f32 	%p13, %f109, 0f00000000;
	.loc 3 85 1
	fma.rn.f32 	%f110, %f98, %f5, %f4;
	.loc 3 86 1
	sub.f32 	%f111, %f5, %f101;
	.loc 3 84 1
	selp.f32 	%f112, %f110, %f143, %p13;
	selp.f32 	%f113, %f111, %f144, %p13;
	selp.f32 	%f114, %f6, %f148, %p13;
	.loc 3 90 1
	mul.f32 	%f115, %f50, %f50;
	.loc 4 2399 3
	div.rn.f32 	%f116, %f25, %f115;
	.loc 3 90 1
	sub.f32 	%f117, %f103, %f4;
	sub.f32 	%f118, %f104, %f5;
	sub.f32 	%f119, %f105, %f6;
	sub.f32 	%f120, %f112, %f4;
	sub.f32 	%f121, %f113, %f5;
	sub.f32 	%f122, %f114, %f6;
	add.f32 	%f123, %f117, %f120;
	add.f32 	%f124, %f118, %f121;
	add.f32 	%f125, %f119, %f122;
	.loc 3 90 1
	fma.rn.f32 	%f126, %f116, %f123, %f27;
	fma.rn.f32 	%f127, %f116, %f124, %f26;
	fma.rn.f32 	%f128, %f116, %f125, %f28;
	.loc 3 91 1
	sub.f32 	%f129, %f104, %f113;
	mul.f32 	%f130, %f129, %f47;
	.loc 4 2399 3
	div.rn.f32 	%f131, %f130, %f50;
	.loc 3 91 1
	add.f32 	%f132, %f126, %f131;
	.loc 3 92 1
	sub.f32 	%f133, %f103, %f112;
	mul.f32 	%f134, %f133, %f47;
	.loc 4 2399 3
	div.rn.f32 	%f135, %f134, %f50;
	.loc 3 92 1
	sub.f32 	%f136, %f127, %f135;
	.loc 3 26 1
	shl.b64 	%rd50, %rd7, 2;
	add.s64 	%rd51, %rd6, %rd50;
	.loc 3 96 1
	st.global.f32 	[%rd51], %f132;
	.loc 3 26 1
	add.s64 	%rd53, %rd5, %rd50;
	.loc 3 97 1
	st.global.f32 	[%rd53], %f136;
	.loc 3 26 1
	add.s64 	%rd55, %rd4, %rd50;
	.loc 3 98 1
	st.global.f32 	[%rd55], %f128;

BB2_10:
	.loc 3 99 2
	ret;
}


`
)
