...
func init() {
	...
	ku1_red.init(1, "ku1_red", "T", func(p *param) {

	})
	kc1_red.init(1, "kc1_red", "T", func(p *param) {
	...
	B_anis.init(3, &globalmesh, "B_anis", "T", "Anisotropy field", func(dst *data.Slice) {
		...
		if !kc1_red.zero() {
			cuda.AddCubicAnisotropy(dst, M.buffer, kc1_red.Gpu3(), AnisC1.Gpu(), AnisC2.Gpu(), regions.Gpu())
		}
	})
