...
// themselves. tryRunGraph captures the torque-evaluation kernel sequences as
...
// Returns true if the current configuration is within the envelope graphs have
...
// on the speedups reported in Fig. 3 of You2026:
...
// such that tryRunGraph then finishes the remaining steps via RunWhile, which
...
// Implements tryRunGraph for Heun (solver 2) FixDt != 0.
...
// Implements tryRunGraph for Heun (solver 2) for FixDt == 0,
...
// Implements tryRunGraph for Bogacki-Shampine (solver 3),
// using the "split graph" approach (see Sec. 4.1 in You2026).
//
// Since the kernel sequence computing the torque is independent of dt and M
...
// Implements tryRunGraph for Runge-Kutta (solver 4),
// using the "split graph" approach (see Sec. 4.1 in You2026).
//
// Since the kernel sequence computing the torque is independent of dt and M
// (only the addresses of the buffers matter, which are fixed), it suffices to
...
// Implements tryRunGraph for Dormand-Prince (solver 5),
// using the "split graph" approach (see Sec. 4.1 in You2026).
//
// Since the kernel sequence computing the torque is independent of dt and M
// (only the addresses of the buffers matter, which are fixed), it suffices to
...
// Implements tryRunGraph for Runge-Kutta-Fehlberg (solver 6),
// using the "split graph" approach (see Sec. 4.1 in You2026).
//
// Since the kernel sequence computing the torque is independent of dt and M
// (only the addresses of the buffers matter, which are fixed), it suffices to
...
// Implements tryRunGraph for Minimize,
// using the "split graph" approach (see Sec. 4.1 in You2026).
//
// Since the kernel sequence computing the torque is independent of dt and M
// (only the addresses of the buffers matter, which are fixed), it suffices to
