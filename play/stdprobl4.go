...
func main() {
	...
	acc := 8
	pbc = [3]int{0, 0, 0}
	kernel := mag.BruteKernel(nimble.PadSize(size, pbc), cellsize, pbc, acc)
...
func update() {
	...
	nimble.AddConst(Heff_[1], B1)
	nimble.AddConst(Heff_[2], B2)
...
func output() {
	...
	if tablef == nil {
		tablef = nimble.OpenFile("stdprobl4/m.table")
