...
func (d *Page) TextBox(id string, value interface{}, extra ...string) string {
	...
	d.addElem(id, e)
	return fmt.Sprintf(`<input type=%v class=TextBox id=%v onchange="notifyel('%v', 'value')" onfocus="notifyfocus('%v')" onblur="notifyblur('%v')" oninput="makered('%v')" %v />`, "text", id, id, id, id, id, cat(extra))
}
