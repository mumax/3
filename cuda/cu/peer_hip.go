...
// This file implements HIP peer device access.
...
func CtxEnablePeerAccess(peer Context) {
	err := Result(C.hipCtxEnablePeerAccess(C.hipCtx_t(unsafe.Pointer(uintptr(peer))), C.uint(0)))
	if err != SUCCESS {
		panic(err)
	}
}
...
func CtxDisablePeerAccess(peer Context) {
	err := Result(C.hipCtxDisablePeerAccess(C.hipCtx_t(unsafe.Pointer(uintptr(peer)))))
	if err != SUCCESS {
		panic(err)
	}
}
...
func DeviceCanAccessPeer(dev, peer Device) bool {
	...
	err := Result(C.hipDeviceCanAccessPeer(&canAccessPeer, C.int(dev), C.int(peer)))
	if err != SUCCESS {
		panic(err)
	}
